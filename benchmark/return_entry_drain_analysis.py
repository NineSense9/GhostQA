"""v0.3.18 outcome gate. Does not choose exploration actions."""
...
OUTCOME_MEANING = {
    "A": (
        "Return-phase entry drain repairs the inspected result-hub return "
        "boundary while preserving directory selectivity, prior transfer, "
        "inspected mechanism cases, and historical regressions"
    ),
    "B": (
        "Safety and protocol pass and the return-entry mechanism engages, "
        "but the full Outcome A gate is incomplete"
    ),
    "C": "unsafe / harmful / protocol-invalid",
    "D": (
        "Safety and protocol pass, but the diagnosed result return-entry "
...
def directory_preserved_v0318(row: dict | None, guard_confirmed, candidate_confirmed) -> bool:
    cell = row or {}
    return bool(
...
def lab_mechanism_gate(facts: dict) -> bool:
    """Preregistered Lab @120 return-entry checks. Button ids stay outside the candidate."""
    return bool(
        facts.get("result_hub_reached")
        and facts.get("false_to_true_at_result")
...
def derive_v0318_outcome(**facts) -> dict:
...
__all__ = [
    ...
    "REGRESSION_APPS",
    "confirmed_of",
    "derive_v0318_outcome",
    "directory_preserved_v0318",
    "lab_mechanism_gate",
    "lost_vs",
    "mechanism_preserved",
    "new_guard_losses",
