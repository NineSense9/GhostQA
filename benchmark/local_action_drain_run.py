"""Exact 12-cell runner for the frozen v0.3.17 local action drain candidate.
...
CANDIDATE = "ghost-structural-local-action-drain-guard"
SEED = 1
...
def make_policy(name: str, seed: int):
    if name == CANDIDATE:
        ...
        return LocalActionDrainGuardGhostPolicy(llm=None)
...
def _check_freezes() -> None:
    for path, label in (
        (CANDIDATE_FREEZE, "v0.3.17 candidate"),
...
def run_cell(app: str, budget: int, out_root: str) -> dict:
    ...
    try:
        ...
        print(
            ...
            f"probes={row.get('local_action_probe_completed_events')} "
            f"promoted={row.get('local_action_promoted_to_child_events')} "
            f"witness_v={row.get('witness_violations')} "
            f"term_v={row.get('terminal_accounting_violations')} "
            f"local_v={row.get('local_action_sequence_accounting_violations')} "
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join("experiments", "runs", "v0317-local-action"))
