"""python -m benchmark.return_entry_drain_reproduce --root experiments/published/return-entry-drain-v0.3.18 --verify"""
...
from benchmark.return_entry_drain_publish import (
    ANALYSIS, CANDIDATE, CELLS, FREEZE, PROTOCOL, PUBLISHED, collect,
)
from benchmark.return_entry_drain_run import APPS, cell_stem
...
def verify(root: str) -> int:
    ...
    if verify_freeze(FREEZE):
        print("v0.3.18 candidate freeze MATCH false")
        return 2
    print("v0.3.18 candidate freeze match true")
    if sha256_file(os.path.join(root, "protocol.json")) != sha256_file(PROTOCOL):
        print("protocol identity MATCH false")
        return 2
    if sha256_file(os.path.join(root, "v0317-return-entry-analysis.json")) != sha256_file(ANALYSIS):
    ...
    if sha256_file(os.path.join(root, "metrics", "safety.json")) != repro.get("safety_sha256"):
        ...
        return 2
    if sha256_file(os.path.join(root, "metrics", "return-entry-audit.json")) != repro.get("return_entry_audit_sha256"):
        print("return entry audit hash MATCH false")
        return 2
    print("mechanism hash match true")
    print("regression hash match true")
    print("safety hash match true")
    print("return entry audit hash match true")
    ...
    if CANDIDATE != "ghost-structural-return-entry-drain-guard":
