"""python -m benchmark.residual_frontier_debt_reproduce --root experiments/published/residual-frontier-debt-v0.3.23 --verify"""
...
from benchmark.residual_frontier_debt_publish import FREEZE, PROTOCOL
from benchmark.residual_frontier_debt_run import CELLS, cell_stem
...
AUDIT = os.path.join(ROOT, "experiments", "validation", "v0.3.23", "v0322-residual-debt-audit.json")
...
def verify(root: str) -> int:
    ...
    for path, label in (
        ...
        (os.path.join(ROOT, FREEZE), "v0.3.23 candidate"),
    ):
    ...
    if sha256_file(os.path.join(root, "v0322-residual-debt-audit.json")) != sha256_file(AUDIT):
        return _fail("audit hash mismatch")
    protocol = _load(os.path.join(root, "protocol.json"))
    if protocol.get("starting_head") != "34e8e3e625eb8fae7f4754bd6e3a1c54839eedd7":
    ...
    derived = derive_v0323_outcome(**(saved.get("facts") or {}))
