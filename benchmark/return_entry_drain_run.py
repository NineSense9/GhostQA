"""Exact 12-cell runner for the frozen v0.3.18 return-entry drain candidate.
...
CANDIDATE = "ghost-structural-return-entry-drain-guard"
SEED = 1
...
def make_policy(name: str, seed: int):
    if name == CANDIDATE:
        ...
        return ReturnEntryDrainGuardGhostPolicy(llm=None)
...
def _check_freezes() -> None:
    for path, label in (
        (CANDIDATE_FREEZE, "v0.3.18 candidate"),
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join("experiments", "runs", "v0318-return-entry"))
