"""New-application generator for the v0.3.26 fresh transfer.

Does not edit the frozen v0.3.20 generator, the v0.3.24 candidate, or the
v0.3.25 applications. Positives are a single spine plus one board workflow.
The score page is a sibling on the chart hub, not on the desk hub.
"""
...
SEED_PREFIX = "ghostqa-v0.3.26:"

FAMILIES = {
    "buggy-ward": {
        "family": "chart_score_sibling", "control_class": "positive", "port": 3971,
        "prefix": "BUG-WD", "brand_pool": ("澄病房", "北廊病区", "拾床记录"),
        "storage_key": "ward_v0326", "fail_error": "ward export dispatch failed",
        "search_error": "ward search query too long", "sync_error": "ward sync handshake failed",
        "assert_prefix": "ward",
    },
    "buggy-harbor": {
        "family": "chart_score_sibling", "control_class": "positive", "port": 3972,
        "prefix": "BUG-HB", "brand_pool": ("澄港池", "北堤泊位", "拾缆台"),
        "storage_key": "harbor_v0326", "fail_error": "harbor export dispatch failed",
        "search_error": "harbor search query too long", "sync_error": "harbor sync handshake failed",
        "assert_prefix": "harbor",
    },
    "buggy-gallery": {
        "family": "chart_score_sibling", "control_class": "positive", "port": 3973,
        "prefix": "BUG-GL", "brand_pool": ("澄展厅", "北廊画档", "拾帧册"),
        "storage_key": "gallery_v0326", "fail_error": "gallery export dispatch failed",
        "search_error": "gallery search query too long", "sync_error": "gallery sync handshake failed",
        "assert_prefix": "gallery",
    },
    "buggy-signal": {
        "family": "chart_score_sibling", "control_class": "positive", "port": 3974,
        "prefix": "BUG-SG", "brand_pool": ("澄信号", "北塔台账", "拾频室"),
        "storage_key": "signal_v0326", "fail_error": "signal export dispatch failed",
        "search_error": "signal search query too long", "sync_error": "signal sync handshake failed",
        "assert_prefix": "signal",
    },
    "buggy-rack": {
        "family": "shallow_rack", "control_class": "catalog", "port": 3975,
        "prefix": "BUG-RK", "brand_pool": ("澄料架", "北仓格口", "拾层"),
        "storage_key": "rack_v0326", "fail_error": "rack export dispatch failed",
        "search_error": "rack search query too long", "sync_error": "rack sync handshake failed",
        "assert_prefix": "rack",
    },
    "buggy-window": {
        "family": "shallow_window", "control_class": "kiosk", "port": 3976,
        "prefix": "BUG-WN", "brand_pool": ("澄窗口", "北厅叫号", "拾次"),
        "storage_key": "window_v0326", "fail_error": "window export dispatch failed",
        "search_error": "window search query too long", "sync_error": "window sync handshake failed",
        "assert_prefix": "window",
    },
}

POSITIVE = ("buggy-ward", "buggy-harbor", "buggy-gallery", "buggy-signal")
NEGATIVE = ("buggy-rack", "buggy-window")
NAMES = {
    "record": ("甲卷", "乙卷", "丙卷", "丁卷"),
    "board": ("早班", "午班", "晚班"),
}
...
def _positive_bugs(prefix, fam):
    ...
    return [
        ...
        _bug(f"{prefix}12", "state inconsistency", "semantic", 3, "分数展示与记录不一致",
             {"assert_id": f"{fam['assert_prefix']}_score_matches"},
             [f"{home}: nav_roster", "roster.html: open_r1", "chart.html: open_finding"], "high"),
    ]
...
def build_spine(app_name: str):
        ...
        b.add(
            f"chart_{item_id}", "chart.html", "chart", "图表", [
                ...
                _link("open_finding", "记分页", "finding.html", "finding",
                      ...
                      guarantees_finding=True, path_id="finding-chart", path_index=0),
    ...
    b.add("desk", "desk.html", "desk", "台面", [
        ...
        _link("open_pad", "附记", "pad.html", "pad"),
    ...
    b.add("finding", "finding.html", "finding", "记分", _finding_controls([
        _back("nav_up_chart_finding", "返回图表", "chart.html?id=r1", "chart_r1"),
    ]), hub_role="finding", obs=["score_shown", "score_live", "scratch_a", "scratch_b"])
    b.add("pad", "pad.html", "pad", "附记", [
        _back("nav_up_desk_pad", "返回台面", "desk.html", "desk"),
    ], hub_role="pad")
...
def build_shelf(app_name: str = "buggy-rack"):
    fam = FAMILIES[app_name]
    b = SpineBuilder(app_name)
    home = [
    ...
    spec = _spec(fam["assert_prefix"])[:1]
    prefix = fam["prefix"]
    assert_prefix = fam["assert_prefix"]
    bugs = [
        _bug(f"{prefix}1", "boundary js error", "js_error", 1, "检索词过长",
             {"error_contains": fam["search_error"]},
             ["index.html: search_box=25 chars", "index.html: btn_query_submit"]),
        _bug(f"{prefix}2", "form validation", "semantic", 2, "空标题仍提示已创建",
             {"assert_id": f"{assert_prefix}_title_required"},
             ["index.html: nav_shelf", "shelf.html: open_a", "settings.html: btn_submit"], "high"),
        _bug(f"{prefix}3", "navigation loop", "nav_loop", 2, "说明与偏好互链",
             {"cycle_urls_contains": "help.html"},
             ["index.html: open_aid", "help.html: open_loop", "settings.html: open_loop"]),
        _bug(f"{prefix}4", "dead action", "dead_action", 2, "草稿点击无效果",
             {"eid": "btn_pin"}, ["index.html: open_aid", "help.html: open_loop", "settings.html: btn_pin"]),
        _bug(f"{prefix}5", "js error", "js_error", 2, "同步异常",
             {"error_contains": fam["sync_error"]},
             ["index.html: open_aid", "help.html: open_loop", "settings.html: btn_probe"]),
        _bug(f"{prefix}6", "http 5xx", "http_error", 2, "导出 500",
             {"error_contains": "/api/export"},
             ["index.html: open_aid", "help.html: open_loop", "settings.html: btn_export"], "high"),
        _bug(f"{prefix}7", "state inconsistency", "semantic", 2, "数量不一致",
             {"assert_id": f"{assert_prefix}_qty_matches"},
             ["index.html: nav_shelf", "shelf.html: open_a"]),
        _bug(f"{prefix}8", "state inconsistency", "semantic", 2, "分数不一致",
             {"assert_id": f"{assert_prefix}_score_matches"},
             ["index.html: nav_shelf"]),
    ]
    return b, topology, model, bugs, spec


def build_counter(app_name: str = "buggy-window"):
    fam = FAMILIES[app_name]
    b = SpineBuilder(app_name)
    home = [
    ...
    prefix = fam["prefix"]
    spec = [
        {"id": f"{assert_prefix}_title_required", "severity": "high", "desc": "标题为空时不应提示已创建",
         "when": {"url_contains": "/settings.html"},
         "assert": {
             "op": "implies",
        ...
        {"id": f"{assert_prefix}_score_matches", "severity": "high", "desc": "展示分数应与记录分数一致",
    ...
    bugs = [
        ...
        _bug(f"{prefix}6", "http 5xx", "http_error", 2, "导出 500",
             {"error_contains": "/api/export"},
             ["index.html: open_aid", "help.html: open_loop", "settings.html: btn_export"], "high"),
        _bug(f"{prefix}7", "state inconsistency", "semantic", 3, "分数不一致",
             {"assert_id": f"{assert_prefix}_score_matches"},
             ["index.html: nav_counter", "counter.html: open_t1"], "high"),
        _bug(f"{prefix}8", "local action", "semantic", 3, "标注不一致",
             {"assert_id": f"{assert_prefix}_note_label"},
