"""v0.3.13 mechanism matrix runner.

Installs the stack and flattening policies in this process only.
Frozen web_runner.py stays byte-identical.
"""
...
DEFAULT_POLICIES = (GUARD, FLAT, STACK)
        ...
        for policy in policies:
            for budget in budgets:
                ...
                print(
                    ...
                    f"push={row.get('suspended_frame_push_events')} "
                    f"resume={row.get('suspended_frame_resume_events')} "
                    f"unwind={row.get('suspended_frame_unwind_events')} "
                    f"depth={row.get('max_suspended_stack_depth')} "
                    f"horizon={row.get('sequence_horizon_reached')} "
                    f"ret={row.get('return_attempt_events')} "
                    f"esc={row.get('return_cycle_escape_events')} "
                    f"viol={row.get('terminal_accounting_violations')} "
