"""Exact 12-cell runner for the frozen v0.3.16 reentry frontier candidate.

Installs the research policy in this process only. Does not rerun v0.3.14 H
or the v0.3.15 matrix. Completed cells are never rerun.
"""
...
CANDIDATE = "ghost-structural-reentry-frontier-guard"
SEED = 1
...
def make_policy(name: str, seed: int):
    if name == CANDIDATE:
        ...
        return ReentryFrontierGuardGhostPolicy(llm=None)
...
def _check_freezes() -> None:
    for path, label in (
        (CANDIDATE_FREEZE, "v0.3.16 candidate"),
...
def run_cell(app: str, budget: int, out_root: str) -> dict:
    ...
    try:
        ...
        print(
            ...
            f"reentry={row.get('early_parent_reentry_events')} "
            f"lease={row.get('local_frontier_lease_granted_events')} "
            f"lease_action={row.get('local_frontier_lease_action_events')} "
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join("experiments", "runs", "v0316-reentry"))
