"""Exact 24-cell runner for the frozen v0.3.24 episode-drain candidate.

Installs the research policy in this process only. Completed cells are never
rerun. Does not rerun v0.3.20 Guard or v0.3.23 references.
"""
...
CANDIDATE_SOURCE = os.path.join(
    "ghostqa", "exploration", "episode_drain_epoch_guard.py")
...
def _check_freezes() -> None:
    ...
    for path, label in (
        (CANDIDATE_FREEZE, "v0.3.24 candidate"),
    ...
    if recorded and sha256_file(os.path.join(ROOT, CANDIDATE_SOURCE)) != recorded:
        raise SystemExit("v0.3.24 candidate source hash mismatch")
    debt = json.load(open(DEBT_FREEZE, encoding="utf-8"))
    debt_hash = ((debt.get("files") or {}).get(DEBT_SOURCE.replace("\\", "/")) or {}).get("sha256")
    if debt_hash and sha256_file(os.path.join(ROOT, DEBT_SOURCE)) != debt_hash:
...
def run_cell(app: str, budget: int, out_root: str) -> dict:
    ...
    try:
        ...
        print(
            ...
            f"revalidated={row.get('local_drain_revalidated_probe_count')} "
            f"invalidated={row.get('local_drain_same_hub_keys_invalidated')} "
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join("experiments", "runs", "v0324-episode-drain"))
