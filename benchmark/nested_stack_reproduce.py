...
from benchmark.nested_stack_publish import (
    ANALYSIS, CANDIDATE_FREEZE, FLATTEN_FREEZE, PROTOCOL, PUBLISHED_ROOT,
    TARGET_FREEZES, build_all,
)
...
def verify(root: str) -> int:
    ...
    flat = verify_freeze(FLATTEN_FREEZE)
    if flat:
        print("v0.3.12 candidate freeze MATCH false")
        for line in flat:
            print(" ", line)
        return 2
    print("v0.3.12 candidate freeze match true")
    cand = verify_freeze(CANDIDATE_FREEZE)
    if cand:
        print("v0.3.13 candidate freeze MATCH false")
        for line in cand:
            print(" ", line)
        return 2
    print("v0.3.13 candidate freeze match true")
    ...
    if proto.get("candidate", {}).get("identity") != "ghost-structural-nested-stack-guard":
        print("protocol identity MATCH false")
        return 2
    ...
    if proto.get("executed") is not False:
        print("protocol identity MATCH false")
        return 2
    if proto.get("historical_outcomes_preserved", {}).get("v0.3.12") != "C":
        print("protocol identity MATCH false")
        return 2
    ...
    pub_analysis = os.path.join(root, "regression-analysis.json")
    if sha256_file(pub_analysis) != sha256_file(ANALYSIS):
        print("regression-analysis MATCH false")
        return 2
    if sha256_file(ANALYSIS) != proto.get("regression_analysis_sha256"):
        print("regression-analysis MATCH false")
        return 2
    print("regression-analysis match true")
    ...
    checks = (
        ...
        ("safety", os.path.join(root, "metrics", "stack-safety.json"), safety),
    )
...
def main(argv=None) -> int:
    ...
    print("pass --verify or --record-clean-clone", file=sys.stderr)
    return 2
