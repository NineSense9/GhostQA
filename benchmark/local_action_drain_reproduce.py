"""python -m benchmark.local_action_drain_reproduce --root experiments/published/local-action-drain-v0.3.17 --verify"""
...
from benchmark.local_action_drain_publish import (
    ANALYSIS, CANDIDATE, CELLS, FREEZE, PROTOCOL, PUBLISHED, collect,
)
from benchmark.local_action_drain_run import APPS, cell_stem
...
def verify(root: str) -> int:
    ...
    regression = _load(os.path.join(root, "metrics", "regression.json"))
    safety = _load(os.path.join(root, "metrics", "safety.json"))
    audit = _load(os.path.join(root, "metrics", "local-action-audit.json"))
    ...
    if sha256_file(os.path.join(root, "metrics", "safety.json")) != repro.get("safety_sha256"):
        ...
        return 2
    if sha256_file(os.path.join(root, "metrics", "local-action-audit.json")) != repro.get("local_action_audit_sha256"):
        print("local action audit hash MATCH false")
        return 2
    print("mechanism hash match true")
    print("regression hash match true")
    print("safety hash match true")
    print("local action audit hash match true")
    ...
    if CANDIDATE != "ghost-structural-local-action-drain-guard":
        print("candidate identity MATCH false")
        return 2
    del regression, safety, audit
    print("OK")
    return 0
