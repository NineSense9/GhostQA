"""One-shot 42-cell runner for the v0.3.26 untried-sibling transfer.

Guard and the new candidate on the new suite, the candidate on the historical
regression list, and the candidate on the four inspected v0.3.25 positives.
Completed cells are not rerun.
"""
...
SEED = 1
CANDIDATE = "ghost-structural-untried-sibling-guard"
GUARD = "ghost-structural-return-guard"
SUITE_FREEZE = os.path.join(
    ROOT, "experiments", "frozen", "v0.3.26-fresh-transfer-suite", "freeze.json")
CANDIDATE_FREEZE = os.path.join(
    ROOT, "experiments", "frozen", "ghost-untried-sibling-v0.3.26", "freeze.json")
...
INSPECTED = ("buggy-clinic", "buggy-dispatch", "buggy-archive", "buggy-fleet")
...
def _policy(name: str, seed: int):
    if name == CANDIDATE:
        ...
        return UntriedSiblingGuardGhostPolicy(llm=None)
...
def run_cell(app: str, policy: str, budget: int, out_root: str) -> None:
    ...
    try:
        ...
        print(
            ...
            f"sibling={row.get('untried_sibling_before_return_selections')} "
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--out", default=os.path.join("experiments", "runs", "v0326-fresh-transfer"))
