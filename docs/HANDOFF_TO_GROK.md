...
>
> **v0.3.18 更新**：Return-Phase Entry Drain。协议 `f3bc961321d7d2058c45346394c0d99d3e4aaac0`。候选冻结 `985f5d118d2c024b6e2ffc45d14929b3f0823b72`。**Outcome C**：结果页 false→true 时开始 drain，`btn_close` 然后 `btn_reopen`，BUG-L9 第 18 步确认，Lab 保住 L1 L2 L8 L9 L10。目录 handoff 0。forum 与 billing strict full transfer 为 true。CRM 28/16，Ops 34/25。Desk / Wiki / BuggyShop 相对 guard 丢失为 0。DeepBench 丢失 BUG-D6、BUG-D7、BUG-D11、BUG-D12、BUG-D13、BUG-D14。RE1–RE16、X1–X10、66429 return-entry traces、66429 local-action traces、37448 reentry traces、19607 handoff traces、5800 return traces 的 failures 为 0。v0.3.17 仍是 Outcome B，v0.3.16 仍是 Outcome B，v0.3.15 仍是 Outcome C，v0.3.14 仍是 Outcome A。不是 fresh 验证，不晋升，产品默认不切。本轮不再加 link drain、文本优先级或结果页特例。`python -m benchmark.return_entry_drain_reproduce --root experiments/published/return-entry-drain-v0.3.18 --verify`
>
> **v0.3.17 更新**：Local Action Drain。协议 `de348adc61fedbffc5b341edea9e432195ae6c86`。候选冻结 `5a46bbab67ba6c3cc56afa807ce2c62de7347ae0`。**Outcome B — partial safe repair**：run 父页面在结构租约之前排空 `btn_staff_note`、`btn_cool`，随后 `open_result_from_run` 仍是结构租约。BUG-L10 第 15 步、BUG-L8 第 14 步、BUG-L1 第 61 步。结果页没有 drain，BUG-L9 仍丢失。目录 handoff 0，提前返回 4，drain 0。forum 与 billing strict full transfer 均为 true。CRM 27/16，Ops 34/25。Desk / DeepBench / Wiki / BuggyShop 相对 guard 丢失为 0。A1–A12、LA1–LA10、I1–I10、66429 local-action traces、37448 reentry traces、H1–H20、19607 handoff traces、5800 return traces 的 failures 为 0。v0.3.16 仍是 Outcome B，v0.3.15 仍是 Outcome C，v0.3.14 仍是 Outcome A。不是 fresh 验证，不晋升，产品默认不切。下一轮若要检验这个候选，需要新的 fresh 目标；本轮不再加 link drain 或文本优先级。`python -m benchmark.local_action_drain_reproduce --root experiments/published/local-action-drain-v0.3.17 --verify`
>
