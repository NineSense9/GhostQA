"""Known-path checks for v0.3.15 apps. Does not run C1, G, or H."""
import json
import os
import socket
import subprocess
...
from benchmark.fresh_handoff_generator import APP_NAMES, ROOT
from benchmark.fresh_handoff_vocab import FAMILIES
...
def test_routes_and_judge_files_hidden(app_server):
    ...
    for hidden in ("/bugs.manifest.json", "/topology.json", "/generation.json", "/spec.json"):
...
def test_declared_hubs_match_live_branch_clicks(app_server):
    ...
    try:
        for node in topo["nodes"]:
            ...
            web._goto(base + url)
            state = web.observe()
            assert is_hub(state) is bool(node["hub"]), (name, node["id"], is_hub(state))
